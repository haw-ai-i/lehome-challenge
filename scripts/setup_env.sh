...
if [[ -n "${LEROBOT_EXTRA}" ]]; then
  log "Using lerobot extra '${LEROBOT_EXTRA}'"
else
  log "Using base lerobot install (no extra)"
fi
...
ENV_LEROBOT_EXTRA_CACHE="${ENV_CACHE_DIR}/lerobot_extra.txt"
...
# Rebuild if the requested lerobot extra differs from the cached one.
if [[ "${recreate}" -eq 0 ]]; then
  mkdir -p "${ENV_CACHE_DIR}"
  _requested_extra="${LEROBOT_EXTRA:-none}"
  _cached_extra=""
  if [[ -f "${ENV_LEROBOT_EXTRA_CACHE}" ]]; then
    _cached_extra="$(tr -d '\n' < "${ENV_LEROBOT_EXTRA_CACHE}")"
  fi
  if [[ "${_cached_extra}" != "${_requested_extra}" ]]; then
    recreate=1
  fi
fi
...
PY
  ...
  printf '%s\n' "${LEROBOT_EXTRA:-none}" > "${ENV_LEROBOT_EXTRA_CACHE}"
else
